...
_Use_decl_annotations_
inline Matrix Matrix::CreateBillboard(const Vector3& object, const Vector3& Camera3DPosition, const Vector3& Camera3DUp, const Vector3* Camera3DForward)
{
    using namespace DirectX;
    XMVECTOR O = XMLoadFloat3(&object);
    XMVECTOR C = XMLoadFloat3(&Camera3DPosition);
    ...
    if (XMVector3Less(N, g_XMEpsilon))
    {
        if (Camera3DForward)
        {
            XMVECTOR F = XMLoadFloat3(Camera3DForward);
            Z = XMVectorNegate(F);
        }
        else
            Z = g_XMNegIdentityR2;
    ...
    XMVECTOR up = XMLoadFloat3(&Camera3DUp);
...
_Use_decl_annotations_
inline Matrix Matrix::CreateConstrainedBillboard(const Vector3& object, const Vector3& Camera3DPosition, const Vector3& rotateAxis,
                                                 const Vector3* Camera3DForward, const Vector3* objectForward)
{
    using namespace DirectX;
