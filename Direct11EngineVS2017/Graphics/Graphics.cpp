...
bool Graphics::InitializeDirectX(HWND hwnd)
{
	...
	scd.BufferDesc.Width = this->window_width;
	scd.BufferDesc.Height = this->window_height;
	...
	hr = this->device->CreateDepthStencilView(depthStencilBuffer.Get(), NULL, depthStencilView.GetAddressOf());
	...
	hr = this->device->CreateRasterizerState(&rasterizerDesc, this->rasterrizerState.GetAddressOf());
	COM_ERROR_IF_FAILED(hr, "Failed to create rasterizer state.");

	// Create Rasterizer State for culling front
	CD3D11_RASTERIZER_DESC rasterizerDesc_CullFront(D3D11_DEFAULT);
	rasterizerDesc_CullFront.CullMode = D3D11_CULL_MODE::D3D11_CULL_FRONT;
	hr = this->device->CreateRasterizerState(&rasterizerDesc_CullFront, this->rasterrizerState_CullFront.GetAddressOf());
	COM_ERROR_IF_FAILED(hr, "Failed to create rasterizer state.");
